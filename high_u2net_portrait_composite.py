...
def main():
    ...
    parser.add_argument('-p',default='./test_data/test_portrait_images/your_portrait_im', help='image path')
    parser.add_argument('-o',default='./test_data/test_portrait_images/u2net_high_results_iter', help='image path')
...
#     image_dir = './test_data/test_portrait_images/your_portrait_im'
    image_dir = args.p
    model_paths = os.listdir('/home/ubuntu/workspace/kobaco/sketchy/U-2-Net/saved_models/u2net_high')
    max_itr = 0
    max_itr_model = None
    for path in model_paths:
        if path == ".ipynb_checkpoints": continue
        prefit = path.split('_')[4]
        if int(prefit) > max_itr:
            max_itr = int(prefit)
            max_itr_model = path
    print(max_itr_model)
    
    prediction_dir = args.o + str(max_itr)
#    prediction_dir = './test_data/test_portrait_images/your_portrait_results'
    
    if(not os.path.exists(prediction_dir)):
        os.mkdir(prediction_dir)

    model_dir = './saved_models/u2net_high/' + max_itr_model
#     model_dir = './saved_models/u2net_portrait/u2net_portrait.pth'
    ...
    test_salobj_dataset = SalObjDataset(img_name_list = img_name_list,
                                        lbl_name_list = [],
                                        transform=transforms.Compose([RescaleT(720),#512
