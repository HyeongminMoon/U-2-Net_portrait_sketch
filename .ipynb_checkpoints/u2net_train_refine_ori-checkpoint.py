...
from data_loader import RandomAffine, RandomJitter, RandomBlur, HeadJitter, RandomAffineJitter
...
### change here #############################
model_name = 'thin2' #'u2net'
out_dir = os.path.join(os.getcwd(),"result_thin2")
batch_size_train = 2

data_dir = os.path.join(os.getcwd(), 'trainset' + os.sep)
tra_image_dir = os.path.join('im2_' + os.sep)
tra_label_dir = os.path.join('gt2_' + os.sep)
# tra_mask_dir = os.path.join('mask_line_add' + os.sep)

val_data_dir = os.path.join(os.getcwd(), 'trainset' + os.sep)
val_image_dir = os.path.join('im2' + os.sep)
...
if pretrained:
    net.load_state_dict(torch.load(
        '/home/ubuntu/workspace/U-2-Net_portrait_sketch/saved_models/u2net_bce_itr_70000_train_0.805340_tar_0.090728.pth', map_location='cpu')
    )
...
val_dataloader = DataLoader(salobj_val_dataset, batch_size=batch_size_val, shuffle=False, num_workers=1)
...
for epoch in range(0, epoch_num):
    ...
    if(not os.path.exists(model_dir)):
        os.mkdir(model_dir)
    if epoch % 20 == 0:
        torch.save(net.state_dict(), model_dir + model_name+"_bce_epoch_%d_train_%3f_tar_%3f.pth" % (epoch, running_loss / ite_num4val, running_tar_loss / ite_num4val))
        running_loss = 0.0
        running_tar_loss = 0.0
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(val_dataloader):
                ...
                # save the output
                os.makedirs(os.path.join(out_dir, str(epoch)), exist_ok=True)
                cv2.imwrite(os.path.join(out_dir,str(epoch),model_name+f"_epoch{epoch}_{str(i).zfill(4)}.png" ),dst)
